...
public class YicesWrapper {
	...
	/**
	 * Set Yices output file and attach a reader to it. For some reason, this
	 * needs to be redone every time a context is deleted.
	 */
	...
	public static void main(String[] args) {
		 ...
		 * String eax = "eax";
		 * 
		 * YicesContext ctx = new YicesContext(); ctx.declareVariable(eax,
		 * makeBVType(32)); ctx.putRetractableAssertion(makeEquality(eax,
		 * makeBVConstant(32, 5))); System.out.println("SAT: " + ctx.check());
		 * ctx.putRetractableAssertion(makeOperation("bv-gt", eax,
		 * makeBVConstant(32, 10))); System.out.println("SAT: " + ctx.check());
		 */
	...
	public static String makeBVMul(String e1, String e2) {
		return makeOperation("bv-unsignedMul", e1, e2);
	}
	...
	/**
	 * Destroy Context sometimes disables output redirection of Yices (bug in
	 * Yices?), so we never destroy contexts for now but reuse them.
	 */
