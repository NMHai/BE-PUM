 ...
 * encapsulate all architecture specific aspects of the analyses, and also
 * supports the conversion of disassembled instructions to RTL statements.
 * 
...
public class Architecture {
	...
	// Variables that can be safely forgotten when crossing instruction
	// boundaries
	...
	private final int IMPLICIT_OPERAND_MATCH_SCORE = 5; // Parameter name
														// matches suffix (AL,
														// AX, EAX, CS, DS, ES)
	private final int OPERAND_TYPE_EXACT_SCORE = 4; // Parameter type matches
													// exactly (i, reg, modrm,
													// mem)
	 ...
	 * looking up RTL descriptions. Looks for opcode name and performs parameter
	 * matching:
	 * 
	 * @param instr
	 *            The instruction to be matches with an SSL prototype
	...
	private SSLInstruction matchInstruction(Instruction instr) {
			...
			// but only do this where it has an effect, otherwise drop it
				...
				for (String repInstr : repInstructions) {
					if (name.startsWith(repInstr)) {
						if (x86instr.hasPrefixREPZ())
							name = "REP" + name;
						if (x86instr.hasPrefixREPNZ())
							name = "REPNE" + name;
						legalREP = true;
						break;
					}
				...
				if (!legalREP)
					logger.info("Ignoring REP prefix of " + name + "!");
			}
		}
		...
		for (SSLInstruction proto : instrList) {
			...
			/*
			 * if (logger.isDebugEnabled()) sb.append(proto.getName() + "=" +
			 * score + " ");
			 */
			if (score > maxScore) {
				maxMatch = proto;
	...
	/**
	 * Returns the RTL sequence which corresponds to the specified assembly
	 * instruction. The instruction is looked up in the library and the RTL
	 * template is instantiated with the instruction's parameters.
	 * 
	 * @param address
	 *            the address of the instruction
	 * @param instr
	 *            the assembly instruction to be translated to RTL
	...
	public StatementSequence getRTLEquivalent(AbsoluteAddress address, Instruction instr) {
		...
		/*
		 * Transform Parameters. If there are excessive Asm operands, skip the
		 * first. This fixes the problem with an implicit EAX operand. Might not
		 * work on other architectures than x86!
		 */
		if (sslInstr != null)
		...
		/*
		 * Assign PC - the PC value in the RTL is that of the next instruction
		 * in Intel assembly
		 */
		...
		if (instrRTL != null) {
			// we need to label only after evaluation, as some instructions
			// might disappear
		...
		// Remove bitranges on LHS and split AssignmentTemplates into memory and
		// variables
	...
	public void parseSSL(String fileName) throws FileNotFoundException, ANTLRException {
		...
		for (Iterator<Map.Entry<String, SSLFunction>> iterator = instrPrototypes.entrySet().iterator(); iterator
			...
			// logger.debug("Converting " + proto.getName() + " = " +
			// proto.getAST().toStringTree());
			...
			if (rtlBody != null) {
				...
				if (proto.getName().startsWith("RET"))
					gotoType = RTLGoto.Type.RETURN;
				else if (proto.getName().startsWith("CALL"))
					gotoType = RTLGoto.Type.CALL;
				else
					gotoType = RTLGoto.Type.JUMP;

				// When there is a %pc assignment as last statement in a
				// sequence, turn it to a GOTO.
				// We cannot do that if it appears earlier, b/c we might lose
