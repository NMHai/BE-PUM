...
public class SSLPreprocessor extends antlr.TreeParser implements SSLParserTokenTypes {
	...
	public Map<String, SSLFunction> getInstructions() {
		return instructions;
	}

	public Map<String, List<AST>> getTables() {
		return tables;
	}
	...
	public final List<AST> table_expr(AST _t) throws RecognitionException {
		...
		case LCURLY: {
			...
			res = new LinkedList<AST>(h); /*
										 * Copy so we don't change the other
										 * table!
										 */
		...
		case QUOTE: {
			...
			_t = _t.getNextSibling();
			res = new LinkedList<AST>();
			res.add(astFactory.dupTree(any));
			break;
		}
		case NAME: {
			n = _t;
			...
			if (tables.containsKey(n.getText())) {
				...
				res.add(n);
				/*
				 * lax specification of SSL seems to allow missing quotes? treat
				 * as string literal. throw new
				 * RecognitionException("Undefined table reference " +
				 * n.getText() + "!");
				 */
			}
	...
	public final RTLExpression rtlExpr(AST _t, int bw) throws RecognitionException {
		...
		RTLExpression[] exprList = new RTLExpression[5]; // Needed for the
															// BUILTIN-rule
		int i = 0; // counter
		int n1 = -1, n2 = -1;
		...
		case CAST: {
			...
			// ret = ExpressionFactory.createCast(e1,
			// ExpressionFactory.createNumber(n1,
			// RTLVariable.UNKNOWN_BITWIDTH));
			ret = e1;
