...
/**
 * A bitrange over another expression. Both start and endbits can be
 * expressions, but the bitwidth can only be determined if start and end
 * position are numbers.
 * 
...
public class RTLBitRange extends AbstractRTLExpression implements RTLExpression, Writable {
	 ...
	 * 
	 * @param startBit
	 *            The positive index of the first bit to set
	 * @param endBit
	 *            The positive index of the last bit to set
	 * @return The bitmask
	 */
	 ...
	 * x[0:7] = y; to x := (x & bitmask) | (size of x)y; x[8:15] = y; to x := (x
	 * & bitmask) | ((size of x)y << 8)
	 * 
	 * @param rhs
	 *            the expression that the inverse should be applied to
	...
	public SetOfVariables getUsedVariables() {
		if (usedVariables == null) {
			...
			if (firstBit != null)
				usedVariables.addAll(firstBit.getUsedVariables());
			if (lastBit != null)
				usedVariables.addAll(lastBit.getUsedVariables());
			if (operand != null)
				usedVariables.addAll(operand.getUsedVariables());
		}
	...
	public SetOfVariables getUsedVariablesOnWrite() {
		if (usedVariablesOnWrite == null) {
			...
			if (firstBit != null)
				usedVariablesOnWrite.addAll(firstBit.getUsedVariables());
			if (lastBit != null)
				usedVariablesOnWrite.addAll(lastBit.getUsedVariables());
	...
	/*
	 * @see
	 * org.jakstab.rtl.expressions.AbstractRTLExpression#inferBitWidth(org.jakstab
