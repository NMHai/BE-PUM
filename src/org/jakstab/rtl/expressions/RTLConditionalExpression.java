...
/**
 * An if-then-else expression similar to the C syntax of
...
public class RTLConditionalExpression extends AbstractRTLExpression implements RTLExpression {
	...
	public RTLExpression evaluate(Context context) {
		...
		if (evaldCondition.equals(ExpressionFactory.FALSE))
			return this.falseExpression.evaluate(context);
		else if (evaldCondition.equals(ExpressionFactory.TRUE))
			return this.trueExpression.evaluate(context);

		assert !(evaldCondition instanceof RTLNumber) : "Numeric condition result is neither TRUE nor FALSE but "
				+ evaldCondition;
