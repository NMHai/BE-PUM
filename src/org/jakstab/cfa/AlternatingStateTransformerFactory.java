...
/**
 * Provides state transformers without assumptions about procedures. Call
 * instructions are treated as push / jmp combinations and return instructions
 * as indirect jumps. If the target of a jump cannot be resolved, plug in values
 * from an under-approximation to create MUST edges.
 * 
 * For regular statements, produces MUST edges if there is an under-approximate
 * witness.
 * 
...
public class AlternatingStateTransformerFactory extends ResolvingTransformerFactory {
	...
	protected void saveNewEdges(Set<CFAEdge> transformers, Location l) {
		...
		for (CFAEdge edge : transformers) {
			boolean found = false;
			// We check for this in the loop, because transformers may contain
			// duplicate edges
			// that only differ in their kind. So we check them against each
			// other for upgrading
					...
					if (existingEdge.getTarget().equals(edge.getTarget())) {
						...
						if (!existingEdge.getKind().equals(edge.getKind())) {
							...
							if (existingEdge.getKind().lessOrEqual(edge.getKind())) {
								...
								// " from " + edge.getKind() + " to " +
								// existingEdge.getKind());
								edge.setKind(existingEdge.getKind());
							} else {
								// Incomparable edge kinds cannot happen with
								// current logic
