...
public interface MapMap<K, L, V> {
	 ...
	 * 
	 * @param keyLeft
	 *            the left key for which all triples should be removed.
	 */
	...
	/**
	 * Returns the number of triples in this map by adding the sizes of all
	 * submaps.
	 * 
	...
	/**
	 * An iterator for MapMaps that behaves differently from regular iterators.
	 * Provides direct accessor methods for the current entry to avoid object
	 * creation.
	 */
	public static interface EntryIterator<K, L, V> {
		 ...
		 *             if there is no current entry and we it move past the end.
		 */
		public void next();
