...
/**
 * A generic class for abstract states that map each variable and memory
 * location to some abstract value. It provides functions for abstract
 * expression evaluation via power set expansion or subexpression joining,
 * getters and setters for variables and memory, as well as Cartesian lattice
 * operators. For this to work, you need to supply it with an
 * AbstractValueFactory that creates abstract elements of the specific abstract
 * domain that should be implemented.
 * 
