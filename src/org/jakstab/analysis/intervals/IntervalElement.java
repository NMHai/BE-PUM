...
/**
 * A reduced signed (twos complement) bitvector interval element with region and
 * stride information. An IntervalElement represents the numbers {region:(left +
 * k*stride) | k \in [0 .. (right-left)/stride]} For intervals of size 1, the
 * stride is 0 by definition. The interval is reduced, i.e. its right limit is
 * included (the size of the interval is a multiple of its stride). All values
 * are represented as signed long integers, with left < right.
 * 
...
public class IntervalElement implements AbstractDomainElement, BitVectorType, Iterable<Long> {
	...
	/**
	 * Widening by extending the interval bounds infinitely into the direction
	 * of the parameter. Takes care to preserve singleton-property of top
	 * elements.
	 * 
	 * @param towards
	 *            the target towards which this element should be widened.
	...
	public IntervalElement widen(IntervalElement towards) {
		...
		// if (this != result) logger.debug("Widening " + this + " to " +
		// result);
		return result;
	}
	...
	public IntervalElement join(LatticeElement lt) {
		...
		if (this.leftOpen() && other.rightOpen())
			return getTop(bitWidth);
		if (other.leftOpen() && this.rightOpen())
			return getTop(bitWidth);
	...
	/**
	 * Addition of two strided interval elements. Code adapted from Gogul
	 * Balakrishnan's thesis.
	 * 
	 * @param op
	...
	public IntervalElement negate() {
		if (left == getTop(bitWidth).left) {
			// If this interval is just the minimum value, it's negation is the
			// same value again in 2s complement.
			if (left == right)
				return this;
			return getTop(bitWidth);
		} else {
	...
	/**
	 * Multiplies two interval elements. 2[0;4] * 1[3;6] = 2[0;24] 0,2,4 *
	 * 3,4,5,6 = 0,6,8,10,12,16,20,24
	 * 
	 * 3[0;9] * 0[6;6] = 18[0;54] 0,3,6,9 * 5 = 0,18,36,54
	 * 
	 * 3[-3;6] * 2[4;8] = 6[-24;48] -3,0,3,6 * 4,6,8 =
	 * -24,-18,-12,0,12,18,24,36,48
	 * 
	 * @param op
	 *            the other interval element to multiply this element with.
	...
	public IntervalElement multiply(AbstractDomainElement other) {
		...
		if (newRegion != MemoryRegion.GLOBAL)
			return top;
		...
		return new IntervalElement(region, b[0], b[3], newStride, newBitWidth);
	}
	...
	public IntervalElement signExtend(int first, int last) {
		...
		if (region == MemoryRegion.GLOBAL) {
			...
			return new IntervalElement(region, left, right, stride, newBitWidth);
		}
		return IntervalElement.getTop(newBitWidth);
	}
