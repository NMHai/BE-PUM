...
public class AbstractReachabilityTree {
	...
	public void addChild(AbstractState parent, AbstractState child) {
		// logger.debug("Adding child " + child.getIdentifier() + " to " +
	...
	public void replace(AbstractState oldState, AbstractState newState) {
		...
		/*
		 * Set<AbstractState> children = new
		 * FastSet<AbstractState>(getChildren(oldState)); for (AbstractState
		 * child : children) { parentChildrenMap.remove(oldState, child);
		 * childParentMap.remove(child); addChild(newState, child); }
		 */
		 ...
		 * 
		 * if (newParent != null) { parentChildrenMap.remove(newParent,
		 * newState); childParentMap.remove(newState); }
		 */
		...
		/*
		 * Set<AbstractState> coveredStates = new
		 * FastSet<AbstractState>(getCoveredStates(oldState));
		 * Set<AbstractState> coveringStates = new
		 * FastSet<AbstractState>(getCoveringStates(oldState)); for
		 * (AbstractState covered : coveredStates) {
		 * coveringMap.remove(oldState, covered); coveredMap.remove(covered,
		 * oldState); addCovered(covered, newState); } for (AbstractState
		 * covering : coveringStates) { coveredMap.remove(oldState, covering);
		 * coveredMap.put(newState, covering); addCovered(newState, covering); }
		 * 
