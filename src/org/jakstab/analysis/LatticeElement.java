...
public interface LatticeElement {
	 ...
	 * of the same lattice. This method has to observe the contract x.join(y) =
	 * y.join(x) and x.lessOrEqual(x.join(y)) && y.lessOrEqual(x.join(y))
	 * 
	 * @param l
	 *            the lattice element to join with.
	 ...
	 * This method observes the contract: x.lessOrEqual(y) && y.lessOrEqual(x)
	 * <=> x.equals(y)
	 * 
	 * @param l
	 *            the lattice element to compare to.
	...
	/**
	 * Check to see whether this element is the bottom element. May be
	 * implemented through a reference check to a unique bottom element.
	 * 
	...
	/**
	 * Returns whether this lattice elements is equivalent to another one. This
	 * method observes the contract: x.lessOrEqual(y) && y.lessOrEqual(x) <=>
	 * x.equals(y) and x.equals(y) <=> y.equals(x)
	 * 
	 * @param o
