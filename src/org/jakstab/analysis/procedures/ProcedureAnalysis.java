...
public class ProcedureAnalysis implements ConfigurableProgramAnalysis {
	...
	public Set<AbstractState> post(AbstractState state, CFAEdge edge, Precision precision) {
		...
		if (edge.getTransformer() instanceof RTLAssume) {
			...
			if (assume.getSource() != null) {
				...
				case RETURN:
					...
					// FastSet<Location>(((Location)edge.getTarget())));
					// FIXME: We need to have a callstack to determine the
					// procedure we are returning to
					// So currently this works only with
					// OptimisticTransformerFactory
					// post =
					// getProcedureState((getCallStackElement().getLabel()))
		...
		// Else just keep the current procedure, includes UnknownProcedureCall
		// edges!
		return Collections.singleton(state);
	}
