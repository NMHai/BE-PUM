...
/**
 * AbstractValueFactory provides a mechanism for generic abstraction of concrete
 * values into an abstract domain. It is used by the ValuationState,
 * VariableValuation, and PartitionedMemory classes.
 * 
 * @author Johannes Kinder
 * @param <A>
 *            The type of elements in the abstract domain
 */
public interface AbstractValueFactory<A extends AbstractValue> {
	 ...
	 * 
	 * @param n
	 *            The concrete number to abstract, must be non-null.
	 ...
	 * 
	 * @param n
	 *            The set of concrete numbers to abstract, must be non-null.
	 * @return the single abstract value that abstracts all concrete numbers in
	 *         the set.
	 */
	public A createAbstractValue(Collection<RTLNumber> numbers);

	/**
	 * Creates the TOP element for this domain with a certain bit width. The
	 * object returned is not guaranteed to be unique. Bitvector type domains
	 * have several "TOP elements", one for each bit width. Joining TOP elements
	 * of several bit widths may cause an exception.
	 * 
	 * @param bitWidth
	 *            the bit width of the top element.
