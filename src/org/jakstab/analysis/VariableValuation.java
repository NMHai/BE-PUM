...
public class VariableValuation<A extends AbstractValue> implements LatticeElement, Iterable<Map.Entry<RTLVariable, A>> {
	...
	public VariableValuation(VariableValuation<A> proto) {
		this(new TreeMap<RTLVariable, A>(proto.aVarVal), proto.valueFactory);
	}

	public VariableValuation(AbstractValueFactory<A> valueFactory) {
		this(new TreeMap<RTLVariable, A>(), valueFactory);
	}
		...
		if (e != null) {
			...
			if (asParent != null && asParent.getOperand() instanceof RTLVariable) {
				...
				if (cValues != null) {
					...
					// logger.debug("Generated abstract value " + e + " for " +
					// var + " from value " + parentVal + " of " + parent);
					return e;
				}
			}
	...
	public void set(RTLVariable var, A value) {
		...
		if (asParent != null && asParent.getOperand() instanceof RTLVariable && value.hasUniqueConcretization()) {
			...
			if (cValues != null) {
				...
				// logger.debug("Setting parent " + parent + " of " + var +
				// " to value " + e);
				set(parent, e);
				return;
			}
