...
/**
 * A set of reached states for use in the CPA algorithm. It can generate
 * lightweight views on parts of its contents much like an SQL database using
 * select(row) and where(row, hasValue).
 * 
...
public class ReachedSet extends AbstractSet<AbstractState> implements Collection<AbstractState> {
	...
	public boolean remove(AbstractState s) {
		if (s instanceof CompositeState)
			return remove((CompositeState) s);
		else
			throw new UnsupportedOperationException();
	}
	...
	public int size() {
		...
		throw new UnsupportedOperationException(
				"Cannot determine reached set size for non-index component restrictions!");
	}
		...
		return new Iterator<AbstractState>() {
			...
			public boolean hasNext() {
				while (next == null) {
					...
					// If we're filtering by indexed component (or not at all),
