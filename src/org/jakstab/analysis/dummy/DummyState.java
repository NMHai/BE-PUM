...
public class DummyState implements AbstractState {
	...
	public Set<Tuple<RTLNumber>> projectionFromConcretization(RTLExpression... expressions) {
		...
		/*
		 * // This makes the dummy analysis resolve both branches of a
		 * conditional jump.
		 * 
		 * Tuple<Set<RTLNumber>> cValues = new
		 * Tuple<Set<RTLNumber>>(expressions.length); for (int i=0;
		 * i<expressions.length; i++) { if (expressions[i] instanceof RTLNumber)
		 * { cValues.set(i, Collections.singleton((RTLNumber)expressions[i])); }
		 * else if (expressions[i].getBitWidth() == 1) { ExpressionFactory
		 * factory = ExpressionFactory.getInstance(); Set<RTLNumber> tf = new
		 * FastSet<RTLNumber>(); tf.add(factory.TRUE); tf.add(factory.FALSE);
		 * cValues.set(i, tf); } else { cValues.set(i, RTLNumber.ALL_NUMBERS); }
		 * }
		 * 
