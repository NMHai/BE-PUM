...
public class NumberElement implements AbstractValue, BitVectorType {
	...
	public NumberElement join(LatticeElement l) {
		...
		// assert other.getBitWidth() == this.getBitWidth() :
		// "Trying to join numberelements of different bitwidth: " + other +
		// " and " + this;
		if (other.getBitWidth() > this.getBitWidth())
			return getTop(other.getBitWidth());
		if (this.getBitWidth() > other.getBitWidth())
			return getTop(this.getBitWidth());
		if (l.isTop() || isTop())
