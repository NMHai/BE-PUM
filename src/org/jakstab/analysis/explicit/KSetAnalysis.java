...
/**
 * A K-Set analysis as used for the example in the VMCAI'09 paper. It represents
 * values as sets of a constant size, merging them to TOP if the bound is
 * exceeded. Supports memory values and register aliasing through the generic
 * VariableValuation class.
 * 
 * For programs with procedure calls, it should be used together with a
 * call-stack analysis for context sensitivity. Otherwise, it will merge the
 * stack contents from different calling contexts, which leads to illegal
 * addresses used as jump targets on return.
 */
	...
	public Set<AbstractState> post(AbstractState state, CFAEdge cfaEdge, Precision precision) {
		...
		return Collections.singleton(statement.accept(new DefaultStatementVisitor<AbstractState>() {
			...
			public AbstractState visit(RTLAlloc stmt) {
				...
				KSet basePointer = new KSet(bound, new BasedNumberElement(newRegion, ExpressionFactory.createNumber(0,
						32)));
