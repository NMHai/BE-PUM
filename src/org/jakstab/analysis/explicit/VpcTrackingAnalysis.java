...
public class VpcTrackingAnalysis implements ConfigurableProgramAnalysis {
	...
	public static JOption<String> vpcName = JOption.create("vpc", "r", "esi",
			"Register to be used as virtual program counter.");
	...
	public Pair<AbstractState, Precision> prec(AbstractState s, Precision precision, ReachedSet reached) {
		...
		if (reached.size() > Math.min(BoundedAddressTracking.varThreshold.getValue(),
			...
			for (RTLVariable v : eprec.varMap.keySet()) {
				// BasedNumberElement currentValue =
				// ((BasedNumberValuation)s).getValueOperand(v);
				...
				if (existingValues.size() > threshold) {
					...
					// This avoids values accumulating at join points (where
					// they are not
			...
			for (EntryIterator<MemoryRegion, Long, BasedNumberElement> entryIt = sStore.entryIterator(); entryIt
				...
				// BasedNumberElement currentValue = entry.getValueOperand();
