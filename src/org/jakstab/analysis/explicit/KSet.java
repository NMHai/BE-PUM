...
public class KSet implements AbstractDomainElement {
	...
	public KSet join(LatticeElement l) {
		...
		if (other.isTop() || this.isBot())
			return other;
		if (this.isTop() || other.isBot())
			return this;
		...
		return result;
		/*
		 * Set<BasedNumberElement> resultSet = new
		 * FastSet<BasedNumberElement>(); int resultBound = Math.max(bound,
		 * other.bound); resultSet.addAll(set); resultSet.addAll(other.set); if
		 * (resultSet.size() > resultBound) { // Create new top element
		 * resultSet = FULL_SET; }
		 * 
	...
	/**
	 * Add an element to this KSet. If it exceeds the bound, set this set to
	 * TOP.
	 * 
	 * @param e
	 *            the element to add
