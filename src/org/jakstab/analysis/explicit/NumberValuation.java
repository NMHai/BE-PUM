...
public final class NumberValuation implements AbstractState {
	...
	public static NumberValuation createInitialState() {
		...
		// initial.setValue(Program.getProgram().getArchitecture().stackPointer(),
		// new NumberElement(espStartValue));
		// push the address of a halt instruction on the stack
		// initial.setValue(factory.createMemoryLocation(espStartValue, 32), new
		// NumberElement(factory.createNumber(0, 32)));
		initial.setValue(ExpressionFactory.createVariable("%DF", 1),
				new NumberElement(ExpressionFactory.createNumber(0, 1)));
		return initial;
	}
	...
	public AbstractState abstractPost(StateTransformer transformer, Precision precision) {
		...
		return statement.accept(new DefaultStatementVisitor<NumberValuation>() {
			...
			private final NumberValuation fallThroughState() {
				return NumberValuation.this; // new
												// NumberValuation(NumberValuation.this);
			}
	...
	public NumberElement getValue(RTLVariable v) {
		if (isTop())
			return NumberElement.getTop(v.getBitWidth());

		if (aVarVal.containsKey(v))
			return aVarVal.get(v);
		else
			return NumberElement.getTop(v.getBitWidth());
	}
	...
	public NumberElement getValue(RTLMemoryLocation m) {
		...
		if (aMemVal.containsKey(m))
			return aMemVal.get(m);
		else {
			...
			if (m.getAddress() instanceof RTLNumber) {
				...
				if (module == null)
					return NumberElement.getTop(m.getBitWidth());
				// only read memory from image if we havn't overapproximated yet
				// or it's a read only section
				if (!dataIsTop || module.isReadOnly(a)) {
					try {
						RTLNumber mValue = module.readMemoryLocation(m);
						// Memory outside the program area is implicitly
						// initialized to top
						if (mValue != null)
	...
	public NumberValuation join(LatticeElement l) {
		...
		if (result.aVarVal.isEmpty() && result.aMemVal.isEmpty() && dataIsTop)
			return TOP;
		return result;
	}
	...
	public boolean lessOrEqual(LatticeElement l) {
		...
		for (Map.Entry<RTLVariable, NumberElement> entry : other.aVarVal.entrySet()) {
			...
			if (!getValue(var).lessOrEqual(v)) {
				// logger.info(var + ": " + getValueOperand(var) +
				// " is not less or equal to " + v);
				return false;
			}
		}
		...
		for (Map.Entry<RTLMemoryLocation, NumberElement> entry : other.aMemVal.entrySet()) {
			...
			if (!getValue(m).lessOrEqual(v)) {
				// logger.info(m + ": " + getValueOperand(m) +
				// " is not less or equal to " + v);
				return false;
			}
		}
	...
	public boolean equals(Object obj) {
		if (!(obj instanceof NumberValuation))
			return false;
