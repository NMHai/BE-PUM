...
public class Elf {
		...
		public String toString() {
			try {
				if (section_strtab == null) {
					final int shstrndx = ehdr.e_shstrndx & 0xffff; // unsigned
																	// short
	...
	public Dynamic[] getDynamicSections(Section section) throws BinaryParseException {
		...
		try {
			...
			int off = 0;
			// We must assume the section is a table ignoring the sh_entsize as
			// it
			// is not
			// set for MIPS.
	...
	public Section[] getSections() throws BinaryParseException {
		if (sections == null) {
			...
			try {
				for (int i = 0; i < length; i++) {
					efile.seek(ehdr.e_shoff + i * (ehdr.e_shentsize & 0xffff)); // unsigned
																				// short
