 ...
 * immutable, i.e. all methods should not modify objects, they should return new
 * object.
 * 
 * Please see Addr32 and Addr64 classes to see how this interface should be
 * extended
 */
public interface IAddress extends Comparable<Object> {
	...
	/**
	 * Adds offset to address and returns new address object which is the result <br>
	 * <br>
	 * Note: This method has an offset limit of Long.MAX and Long.MIN, which
	 * under some addressing schems may impose an unnesseary limitation, see
	 * <code>IAddressa.add(BigInteger offset)</code> to handle larger offsets.
	 * 
	 * @param offset
	 *            to add
	 * @return the new address
	...
	/**
	 * Returns maximal offset possible for address. The offset should be
	 * Identicall for all addresses of given class.
	 * 
	 ...
	 * @param obj
	 *            the other object
	 * @return <code>true</code> if the addresses are equivalent, and
	...
	/**
	 * Converts address to the hex representation with '0x' prefix and with all
	 * leading zeros. The length of returned string should be the same for all
	 * addresses of given class. I.e. 10 for 32-bit addresses and 18 for 64-bit
	 * addresses
	 */
	String toHexAddressString();

	/**
	 * Converts address to the binary representation with '0b' prefix and with
	 * all leading zeros. The length of returned string should be the same for
	 * all addresses of given class. I.e. 34 for 32-bit addresses and 66 for
	 * 64-bit addresses
	 */
