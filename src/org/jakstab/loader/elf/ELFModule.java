...
public class ELFModule implements ExecutableImage {
	...
	public ELFModule(File moduleFile, Architecture architecture) throws IOException, BinaryParseException {
		...
		for (int i = 0; i < elf.getSymtabSymbols().length; i++) {
			...
			if (s.st_type() == Elf.Symbol.STT_FUNC) {
				// logger.info(i + ":\t" + s + "\tshndx: " + s.st_shndx +
				// "\tbind: " + s.st_bind() + "\tSize: " + s.st_size +
				// "\tInfo: " + s.st_info +
				// "\tType: " + s.st_type() + "\tValue: 0x" +
				// Long.toHexString(s.st_value.getValueOperand().longValue()));
				if (s.st_shndx > 0) {
					// logger.info(s + " " + "\t Addr: " +
		...
		while (true) {
			...
			// r_offset is at 0, r_info at 4. r_info is an integer containing
			// the symbol index
			...
			// String functionName = elf.getDynamicSymbols()[(symbolTableOff /
			// 8)].toString();
			...
			// Now skip the following jump to PLT0 (a call to the dynamic
			// loader)
			pltIdx += instr.getSize();
			instr = disasm.decodeInstruction(pltIdx);
	...
	/*
	 * @see org.jakstab.loader.ExecutableImage#getFilePointer(org.jakstab.asm.
	 * AbsoluteAddress)
	 */
	@Override
	public long getFilePointer(AbsoluteAddress va) {
		...
		return -1;
		// throw new RuntimeException("Virtual address " + va +
		// " matches no section?");
	}
	...
	/*
	 * @see
	 * org.jakstab.loader.ExecutableImage#isCodeArea(org.jakstab.asm.AbsoluteAddress
	 * )
	 */
	@Override
