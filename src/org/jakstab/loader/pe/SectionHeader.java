...
/**
 * This is the structure used to parse a PE/COFF Section Header. Sections must
 * appear in ascending order of virtual address, and must be consecutive in the
 * virtual address space. Their virtual addresses must be multiples of
 * SectionAlignment in the PE header.
 * 
 * Also, the PointerToRawData and SizeOfRawData fields must be multiples of
 * FileAlignment in the PE header. If (SectionAlignment < PageSize) then
...
public class SectionHeader {
	...
	// Virtual size and address are not final to allow relocation of sections in
	// objs
	...
	public final long PointerToRelocations; // 4bytes (file pointer) (object
											// only)
	...
	private static long LOWER_LIMIT_ALLIGH = 0x200;
...
//		Thus, it's a legitimate way to map the header.
		long temp = in.readDWORD();
		PointerToRawData = temp < LOWER_LIMIT_ALLIGH ? 0:temp;
//		PointerToRawData = temp;		
		PointerToRelocations = in.readDWORD();
		PointerToLinenumbers = in.readDWORD();
		NumberOfRelocations = in.readWORD();
