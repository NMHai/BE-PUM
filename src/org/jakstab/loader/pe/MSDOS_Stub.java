...
/**
 * Ths MS-DOS stub starts a PE/COFF file. The only relevant field in this
 * structure is the NewFileHeaderAddress, which will be at offset 0x3C to tell
 * the start of the file. The value here will be the file offset to the PE
 * signature, which is followed by the PE header. Changed to throw a
 * BinaryParseException for MSDOS-executables
 * 
...
public class MSDOS_Stub {
	...
	// (dump data because file header address starts at byte 60th - 0x3c)
	private byte[] data1; // 58 bytes
	private long NewFileHeaderAddress; // 4 bytes
	// (dump data between file header address inducator and file start)
