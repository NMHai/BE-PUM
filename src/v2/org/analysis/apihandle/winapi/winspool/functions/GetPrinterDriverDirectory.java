 ...
 * File name: GetPrinterDriverDirectory.java
 * Author: Vinh Le
 */
...
public class GetPrinterDriverDirectory extends WinspoolAPI {
	public GetPrinterDriverDirectory () {
		super();
		NUM_OF_PARMS = 6;
	}
	...
	public void execute() {
		...
		byte[] pDriverDirectory = null;
		if ( t3 != 0L ) pDriverDirectory = new byte[(int) t4];
		for (int i = 0; i < pDriverDirectory.length; i++) {
			pDriverDirectory [i] = (byte) ((LongValue) memory.getByteMemoryValue (t3)).getValue();
			t3 += 1;
		}
		...
		int ret = WinspoolDLL.INSTANCE.GetPrinterDriverDirectory (pName, pEnvironment, Level, pDriverDirectory, cbBuf, pcbNeeded);
		...
		for (int i = 0; i < pDriverDirectory.length; i++) {
			memory.setByteMemoryValue (t3, new LongValue(pDriverDirectory [i]));
			t3 += 1;
