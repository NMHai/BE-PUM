...
public class EnumMonitors extends WinspoolAPI {
	public EnumMonitors () {
		super();
		NUM_OF_PARMS = 6;
	}
	...
	public void execute() {
		...
		byte[] pMonitors = null;
		if ( t2 != 0L ) pMonitors = new byte[(int) t3];
		for (int i = 0; i < pMonitors.length; i++) {
			pMonitors [i] = (byte) ((LongValue) memory.getByteMemoryValue (t2)).getValue();
			t2 += 1;
		}
		...
		int ret = WinspoolDLL.INSTANCE.EnumMonitors (pName, Level, pMonitors, cbBuf, pcbNeeded, pcReturned);
		...
		for (int i = 0; i < pMonitors.length; i++) {
			memory.setByteMemoryValue (t2, new LongValue(pMonitors [i]));
			t2 += 1;
