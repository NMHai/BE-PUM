...
public class AddJob extends WinspoolAPI {
	public AddJob () {
		super();
		NUM_OF_PARMS = 5;
	}
	...
	public void execute() {
		...
		byte[] pData = null;
		if ( t2 != 0L ) pData = new byte[(int) t3];
		for (int i = 0; i < pData.length; i++) {
			pData [i] = (byte) ((LongValue) memory.getByteMemoryValue (t2)).getValue();
			t2 += 1;
		}
		...
		int ret = WinspoolDLL.INSTANCE.AddJob (hPrinter, Level, pData, cbBuf, pcbNeeded);
		...
		for (int i = 0; i < pData.length; i++) {
			memory.setByteMemoryValue (t2, new LongValue(pData [i]));
			t2 += 1;
