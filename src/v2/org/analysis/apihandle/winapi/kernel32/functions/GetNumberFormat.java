 ...
 * File name: GetNumberFormat.java
 * Author: Vinh Le
 */
...
public class GetNumberFormat extends Kernel32API {
	...
	public void execute() {
		...
		for (int i = 0; i < lpNumberStr.length; i++) {
			lpNumberStr [i] = (char) ((LongValue) memory.getByteMemoryValue (t4)).getValue();
			t4 += 1;
		}
		int cchNumber = (int) t5;

		// Step 3: call API function
		int ret = Kernel32DLL.INSTANCE.GetNumberFormat (Locale, dwFlags, lpValue, lpFormat, lpNumberStr, cchNumber);
		...
		for (int i = 0; i < lpNumberStr.length; i++) {
			memory.setByteMemoryValue (t4, new LongValue(lpNumberStr [i]));
			t4 += 1;
		}
	}
}