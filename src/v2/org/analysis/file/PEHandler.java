 ...
 * 
 */
public class PEHandler extends AbstractCOFFModule {
	...
	private final PESymbolHandler symbolFinder;

	/**
	 * Parses a PEModule from a specified file
	 */
	public PEHandler(File peFile) throws IOException, BinaryParseException {
	...
	public long getCodeBaseAddress() {
		for (int i = 0; i < this.section_headers.length; i++) {
			if (this.section_headers[i].getName().toLowerCase().equals("code"))
				return this.section_headers[i].VirtualAddress;
		}
